...
let make = () => {
  let ({name, email, password, passwordConfirm, error}, dispatch) =
    React.useReducer(reducer, {email: "", name: "", password: "", passwordConfirm: "", error: ""});
  <div className="bg-indigo-100 min-h-screen flex">
    <form
      ...
      <h1 className="text-center text-xl px-24 font-semibold uppercase mb-4"> "CADASTRO"->React.string </h1>
      {error != ""
      ...
      <button
        ...
        onClick={e => {
          ...
          |> Js.Promise.then_(v => {
               let resp: SignupMutation.Mutation.Operation.response = Obj.magic(v);
               switch (resp.register) {
               | Some(register) =>
                 Dom.Storage.setItem("monis-app-token", register.token, Dom.Storage.localStorage);
