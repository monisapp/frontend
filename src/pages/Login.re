...
[@react.component]
let make = () => {
  let ({email, password, error}, dispatch) = React.useReducer(reducer, {email: "", password: "", error: ""});
  <div className="bg-indigo-100 min-h-screen flex">
    <form className="m-auto shadow bg-white p-16">
      <h1 className="text-center text-xl px-12 font-semibold mb-4"> {j|Faça login na sua conta|j}->React.string </h1>
      {error != ""
         ? <p className="bg-red-100 border border-red-700 text-red-700 py-2 px-4 rounded"> error->React.string </p>
         : React.null}
      <Input
        fullWidth=true
        id="email"
      ...
      <button
        ...
        onClick={e => {
          ...
          |> Js.Promise.then_(v => {
               let resp: LoginMutation.Mutation.Operation.response = Obj.magic(v);
               switch (resp.login) {
               | Some({token}) =>
                 Dom.Storage.(setItem("monis-app-token", token, localStorage));
